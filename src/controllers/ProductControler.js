...
let handleGetAllProductadmin = async (request, response) => {
  try {
    let product = await ProductService.getAllProductadmin();
    return response.status(200).json(product);
  } catch (error) {
    return response.status(400).json(error);
  }
...
let handleGetWarehouseQuantity = async (request, response) => {
  try {
    let quantity = await ProductService.getWarehouseQuantity(request.body);
    return response.status(200).json(quantity);
  } catch (error) {
    return response.status(400).json(error);
  }
};
let handleDeleteProductbystatus = async (request, response) => {
  try {
    let id = request.params.productId;
    let product = await ProductService.deleteProductbystatus(id);
    return response.status(200).json(product);
  } catch (error) {
    return response.status(400).json(error);
  }
...
module.exports = {
  ...
  handleCreateOpttion,
  handleGetWarehouseQuantity,
  handleDeleteProductbystatus,
};
